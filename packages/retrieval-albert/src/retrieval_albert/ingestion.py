"""Document ingestion into Albert collections.

Provides functions to create collections, upload documents, and manage
the ingestion lifecycle via the Albert API.
"""
...
def ingest_documents(
    ...
    *,
    ...
    """
    config = get_config()
    chunk_size = chunk_size if chunk_size is not None else config.chunking.chunk_size
    chunk_overlap = (
        chunk_overlap if chunk_overlap is not None else config.chunking.chunk_overlap
    )
    ...
    logger.info(
        "Ingested %d documents into collection %s", len(document_ids), collection_id
    )
    return document_ids


def delete_collection(client: AlbertClient, collection_id: int) -> None:
