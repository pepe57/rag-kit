...
class AlbertPipeline(RAGPipeline):
    ...
    def _get_chunks(
        ...
        query: str,
        **kwargs: object,
    ) -> tuple[list[Any], list[int | str], list[str], list[dict], list[dict]]:
    ...
    def retrieve_chunks(self, query: str, **kwargs: object) -> list[Any]:
        ...
        Runs the same expand → search → rerank pipeline as
        :meth:`process_query` but returns raw chunks instead of a formatted
        string.  Callers can inspect retrieval quality without having to
        access internal attributes or re-run the pipeline a second time.

        Useful for evaluation: the Inspect AI solver calls this once, extracts
        chunk texts for recall/precision scoring, and formats the context
        locally — a single Albert API call instead of two.

        Args:
            query: User query.
            **kwargs: Same options as :meth:`process_query`.

        Returns:
            List of retrieved :class:`~rag_facile.core.RetrievedChunk` dicts.
            Empty list if no collections are configured or no results found.
        """
